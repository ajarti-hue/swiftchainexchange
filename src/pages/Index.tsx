...
import { Gift, Bitcoin, Phone, MessageCircle, Shield, CheckCircle, User, LogIn } from "lucide-react";
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Top Bar */}
      <div className="mx-auto max-w-lg px-4 pt-3 flex justify-end">
        {user ? (
          <button
            onClick={() => navigate("/account")}
            className="flex items-center gap-1.5 rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary hover:bg-primary hover:text-primary-foreground transition-all"
          >
      ...
      <div className="relative overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
        <div className="mx-auto max-w-lg px-4 py-12 text-center">
          <img src={logo} alt="SwiftChain X" className="mx-auto mb-6 h-24 w-24 rounded-2xl object-cover shadow-[var(--shadow-card)]" />
          <h1 className="font-display text-3xl font-bold text-foreground mb-2">SwiftChain X</h1>
          <p className="text-muted-foreground text-sm mb-3">Fast & secure gift card and crypto trading. Choose what you'd like to do below.</p>
          <div className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
            <Shield size={12} /> Insured & Verified Exchanges
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-lg px-4 -mt-2 pb-0">
        <div className="grid gap-4">
          <TradeCard icon={<Gift size={24} />} title="Trade Gift Card" description="Buy or sell gift cards — Amazon, iTunes, Google Play & more" onClick={() => navigate("/gift-card")} />
          <TradeCard icon={<Bitcoin size={24} />} title="Trade Crypto" description="Buy or sell Bitcoin, Ethereum, USDT and other cryptocurrencies" onClick={() => navigate("/crypto")} />
        </div>
        ...
        <div className="mt-6 relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-card via-card to-primary/5 p-6 shadow-[var(--shadow-card)]">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed mb-3">🔥 Check our latest rates on WhatsApp before you trade! Never miss a great deal.</p>
              <a href="https://whatsapp.com/channel/0029Vb7LE6T89ingo3wmca3s" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-[hsl(142_70%_45%)] px-3.5 py-2 text-xs font-semibold text-white shadow-md transition-all hover:opacity-90">
        ...
        <div className="mt-6 rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="grid grid-cols-2 gap-3">
            {["Personally verified trades", "Insured crypto exchanges", "Competitive rates daily", "Fast WhatsApp support"].map((item) => (
